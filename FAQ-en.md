...
There is no need to download a separate mod for this button now – this functionality has been added to the vanilla game!
...
> [!NOTE]  
> If for some reason Telegram doesn't open the file, click on "⋮", then "Save to Downloads", open a file explorer, go to Download/Telegram folder (for people with certain third-party clients the folder might be called Download/AyuGram or similar) and open the mod file from there.

> [!NOTE]  
> Please note that some file managers don't see Null's Brawl supporting its file format. You can try opening the file with another file manager in this case, such as:
> - <details><summary>Material Files:</summary> The best amongst the ones available on Google Play while being open source and having Material You interface.</details>  
> 
...
> 
> - <details><summary>MT Manager:</summary> The most feature packed manager, some features of which are only for premium users. If the maximum functionality is all that matters to you, then it's the best choice.</details>  
> 
...
### How to make mods?  
Everything is described in the [official manual](/MANUAL-en.md) written in Russian (it'll be available in English someday). Be sure to read it from cover to cover!
