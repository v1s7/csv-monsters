...
А по поводу v2, так как суперселл активно начали использовать FlatBuffer – под раздачу попал и .sc, таким образом получился SC2. По итогу SC2 – всё тот же старый добрый SC, но который был запихнут во FlatBuffer.
...
## Шрифты
Среди них есть только 2 основный формата: .ttf и .otf, в самом Null's Brawl используется только первый, поэтому второй вряд-ли поддерживается игрой. Почитать об их отличиях подробнее можете здесь: https://old.reddit.com/r/typography/comments/ci4nwk/otf_vs_ttf/
