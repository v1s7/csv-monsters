...
Позволяет открывать и просматривать файлы SC (и v1, и v2) и SCTX. Есть возможность сохранять разные файлы игры в формате SC v1.  

Вы можете использовать SC Editor для проверки работоспособности ваших изменений в SC файлах. А если вам нужно отрендерить объект в картинку или видео, он справится и с этим, так что он также полезен для контент-мейкеров.  
  
Ну а если вы умеете писать на Java, то для вас потрясающие новости: SC Editor собран из модулей, а значит вы можете взять эти модули и использовать их в своих целях. Например, вы можете написать приложение, которое будет открывать SC2 файлы и сохранять их в старом формате SC. Небольшой демонстрационный проект, который я описал выше: https://github.com/danila-schelkov/sc-reassembler
